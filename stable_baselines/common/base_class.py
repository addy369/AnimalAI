...
class BaseRLModel(ABC):
    ...
    def __init__(self, policy, env, verbose=0, *, requires_vec_env, policy_base,
        ...
        self.sess = None
        self.params = None
        self.initial_state = None  # my
        self.n_batch = None    # my
        self.nminibatches = None  # my
        self.n_steps = None  # my
        self.seed = seed
    ...
    def pretrain(self, dataset, n_epochs=10, learning_rate=1e-4,
        ...
        for epoch_idx in range(int(n_epochs)):
            ...
            for _ in range(len(dataset.train_loader)):
                ...
                #train_loss_, _ = self.sess.run([loss, optim_op], feed_dict)   # why did he not remove this??
                train_loss += train_loss_

            train_loss /= len(dataset.train_loader)
            #print('Loss at {} epoch: {}.'.format(epoch_idx, train_loss))
            ...
            if self.verbose > 0 and (epoch_idx + 1) % val_interval == 0:
                ...
                #print('Valid Loss at {} epoch: {}.'.format(epoch_idx, val_loss))
                end_time = time.time()
                if self.verbose > 0:
        ...
        np.save('training_loss', np.array(train_loss_vec))
        plt.plot(train_loss_vec)
        plt.savefig('train_graph.png', figsize=(20,15))

        plt.clf()

        np.save('validation_loss', np.array(val_loss_vec))
        plt.plot(val_loss_vec)
        plt.savefig('validation_graph.png', figsize=(20,15))
        plt.clf() 
