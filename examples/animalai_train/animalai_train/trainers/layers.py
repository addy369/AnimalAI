...
def fc2(x, units, scope, activate=True, gain=1.):

def conv2d_block(x):

    h1 = conv2d(x, 'conv1', nfilters=16, nkernel=3, stride=1, pad='SAME')
    h1 = max_pool2d(h1, nkernel=3, stride=1, pad='SAME'):

    h2 = conv2d(h1, 'conv2', nfilters=32, nkernel=3, stride=1, pad='VALID')
    h2 = max_pool2d(h2, nkernel=3, stride=1, pad='SAME'):
    h3 = conv2d(h2, 'conv3', nfilters=64, nkernel=3, stride=1, pad='VALID')
    h3 = max_pool2d(h3, nkernel=3, stride=1, pad='SAME'):
    h4 = conv2d(h3, 'conv4', nfilters=128, nkernel=3, stride=1, pad='VALID')
    
    h4_flat = tf.layers.flatten(h4)

    return lrelu(
            fc(h4_flat, 1024, 'fc1', activate=False))#Check activation_function
